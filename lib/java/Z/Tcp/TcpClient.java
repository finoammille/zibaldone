...
public class TcpClient {
    ...
    private int port;
    public TcpClient(String remoteAddr, int port)
    {
        this.remoteAddr=remoteAddr;
        this.port=port;
    }
    public TcpConnHandler Connect()
    {
        InetAddress srcIpAddr;
        try {
            srcIpAddr = InetAddress.getByName(remoteAddr);
        } catch(UnknownHostException ex) {
            LOG.ziblog(LOG.LogLev.ERR, "Connect - UnknownHostException(%s)", ex.getMessage());
            return null;
        } catch(SecurityException ex) {
            LOG.ziblog(LOG.LogLev.ERR, "Connect - SecurityException(%s)", ex.getMessage());
            return null;
        }
        Socket connSock;
        try {
            connSock=new Socket(srcIpAddr, port);
        } catch(IOException ex) {
            LOG.ziblog(LOG.LogLev.ERR, "Connect - IOException(%s)", ex.getMessage());
            return null;
        } catch(SecurityException ex) {
            LOG.ziblog(LOG.LogLev.ERR, "Connect - SecurityException(%s)", ex.getMessage());
            return null;
        } catch(IllegalArgumentException ex) {
            LOG.ziblog(LOG.LogLev.ERR, "Connect - SocketTimeoutException(%s)", ex.getMessage());
            return null;
        } catch(NullPointerException ex) {
            LOG.ziblog(LOG.LogLev.ERR, "Connect - IllegalBlockingModeException(%s)", ex.getMessage());
            return null;
        }
