...
{
    if(_lock==0) {
        HANDLE _firstTimeLock=CreateMutex(NULL, FALSE, NULL);
        InterlockedCompareExchangePointer(&_lock, _firstTimeLock, NULL);//atomic compare and exchange only if _lock not initialized
        if(_lock!=_firstTimeLock) CloseHandle(_firstTimeLock);
    }
